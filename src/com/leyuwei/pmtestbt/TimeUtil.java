...
public class TimeUtil {  
    ...
    public static Date datePlusOneSec(Date origDate){
        Date newDate = new Date(origDate.getTime() + 1000);
        return newDate;
    }
    
    public static boolean isTimeOverDay(Date origDate, Date newDate){
    	SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");
    	String date1,date2;
    	date1 = df.format(origDate);
    	date2 = df.format(newDate);
    	if(!date1.split("-")[2].equals(date2.split("-")[2]))
    		return true;
		return false;
    }
